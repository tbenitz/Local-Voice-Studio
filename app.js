...
function wireUi(){
  ...
  els.speedRange.addEventListener('input',()=>els.speedOut.value=`${Number(els.speedRange.value).toFixed(2)}x`);
...
function progressHandler(p){
  ...
  const file=p?.file||p?.name||p?.part||'';
  if((status==='progress'||status==='progress_total') && Number.isFinite(p.loaded) && Number.isFinite(p.total) && p.total>0){
    const pct=Math.max(0,Math.min(100,p.loaded/p.total*100));
    const label=file?`Downloading ${String(file).split('/').pop()}… ${pct.toFixed(0)}%`:`Downloading model… ${pct.toFixed(0)}%`;
    setModelProgress(Math.max(3,Math.min(94,pct)),label); setBusyProgress(Math.max(3,Math.min(94,pct)),label);
  } else if(status==='phase'){
    setModelProgress(p.phase==='runtime'?3:p.phase==='processor'?7:p.phase==='model'?12:95,p.text||'Preparing AI…');
    setBusyProgress(p.phase==='runtime'?3:p.phase==='processor'?7:p.phase==='model'?12:95,p.text||'Preparing AI…');
    ...
    setModelProgress(97,'Compiling model for this Chromebook…'); setBusyProgress(97,'Compiling AI sessions…');
  } else if(status==='load-fallback'){
    toast(`WebGPU unavailable; using WASM: ${p.reason||''}`,'warn');
  } else if(status==='synth-progress'){
    setBusyText(`Generating speech chunk ${p.index} of ${p.count}…`);
...
async function initEngine({silent=false}={}){
  ...
  try{
    await requestPersistentStorage(false);
    state.voiceStore ||= new IndexedDbVoiceStore({databaseName:VOICE_DB});
    const tts=await LocalTTS.create({device:state.settings.backend,voiceStore:state.voiceStore,maxChunkLength:Number(state.settings.chunkLength),onProgress:progressHandler});
    state.tts=tts; state.ready=true; state.renderCount=0;
    localStorage.setItem(MODEL_READY_KEY,'1');
    els.diagEngine.textContent='Chatterbox ONNX (Q4)'; els.diagBackend.textContent=tts.device; els.diagRenders.textContent='0';
    els.setupCard.querySelector('h2').textContent='Offline AI is installed';
    ...
    state.ready=false;
    try{await state.tts?.dispose?.();}catch{} state.tts=null;
...
function explainLoadError(e){
  ...
  if(/WebGPU|GPU|device/i.test(m)) return new Error(`${m}\n\nTry Settings → Preferred backend → WASM if this Chromebook WebGPU driver cannot load the model.`);
...
async function disposeEngine(notify=false){
  ...
  state.tts=null;state.ready=false;state.renderCount=0;
...
async function exportVoice(name){
  ...
  try{
    const payload={format:'local-voice-studio-clone',version:2,name,exportedAt:Date.now(),profile:{sampleRate:embedding.sampleRate,audio:packTyped(embedding.samples)},meta:await serializeMeta(meta)};
...
async function importVoice(file){
  ...
  try{
    ...
    if(pkg?.format!=='local-voice-studio-clone'||pkg.version!==2||!pkg.profile?.audio) throw new Error('This clone was made by an older/incompatible build. Re-create it with this fixed version.');
    const profile={samples:unpackTyped(pkg.profile.audio,'float32'),sampleRate:Number(pkg.profile.sampleRate)||24000,createdAt:Date.now(),engine:'chatterbox-reference'};
    if(!state.voiceStore) state.voiceStore=new IndexedDbVoiceStore({databaseName:VOICE_DB});
    ...
    await state.voiceStore.save(name,profile);if(pkg.meta) await putVoiceMeta(name,{...(await deserializeMeta(pkg.meta)),name});
...
async function requestPersistentStorage(notify=true){
  ...
  try{const ok=await navigator.storage.persist();if(notify)toast(ok?'Chrome will protect this app offline data when possible.':'Chrome did not grant persistent storage; the app still works, but storage may be evicted under pressure.',ok?'success':'warn');return ok;}catch{return false;}
}
